...
def visualize_comparison(G, mst_predicted, mst_true, save_filename=None):
    ...
    if save_filename is None:
        ...
        save_dir = Path.cwd()  # Directory corrente

        # Prova anche sul Desktop se esiste
        desktop = Path.home() / "Desktop"
        if desktop.exists():
            save_dir = desktop

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_filename = save_dir / f"mst_comparison_{timestamp}.png"

    save_filename = str(save_filename)

    # Salva con diversi formati per sicurezza
    try:
        plt.savefig(save_filename, dpi=300, bbox_inches='tight', format='png')
        print(f"\n✓ Immagine salvata in: {save_filename}")
        ...
        if os.path.exists(save_filename):
            ...
            print(f"  ATTENZIONE: Il file non sembra esistere!")

    except Exception as e:
        print(f"\n✗ Errore nel salvataggio: {e}")
        # Prova percorso alternativo
        alt_filename = f"mst_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        try:
            plt.savefig(alt_filename)
            print(f"  Salvato con nome alternativo: {alt_filename}")
        except:
            print(f"  Impossibile salvare il file!")

    # Se matplotlib.use('Agg') è attivo, plt.show() non mostrerà nulla
    # plt.show()  # Commentato perché usiamo backend Agg
...
def test_on_multiple_graphs(predictor):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = f"mst_results_{timestamp}"
    os.makedirs(results_dir, exist_ok=True)

    # Stampa percorso assoluto
    abs_path = os.path.abspath(results_dir)
    print(f"\n✓ Cartella risultati creata in: {abs_path}")
    ...
    for name, graph_type, n_nodes in test_configs:
        ...
        if info['success']:
            ...
            filename = os.path.join(results_dir, f"{name.replace(' ', '_')}.png")
            visualize_comparison(G, mst_pred, mst_true, filename)
    ...
    summary_text = []
    summary_text.append("MST MODEL TEST REPORT")
    summary_text.append("="*60)
    summary_text.append(f"Data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    summary_text.append(f"Modello: final_direct_mst_model.pth")
    summary_text.append("")
    summary_text.append("RISULTATI DETTAGLIATI")
    summary_text.append("-"*60)
    summary_text.append("")

    for r in results:
        line = f"{r['name']:<25} {r['nodes']:<8} {r['edges']:<8} {r['quality_gap']*100:<10.1f} {r['edge_accuracy']*100:<10.1f}"
        print(line)

        summary_text.append(f"{r['name']}")
        summary_text.append(f"  Nodi: {r['nodes']}")
        summary_text.append(f"  Edges: {r['edges']}")
        summary_text.append(f"  Gap di qualità: {r['quality_gap']*100:.2f}%")
        summary_text.append(f"  Accuratezza edges: {r['edge_accuracy']*100:.2f}%")
        summary_text.append("")
    ...
    summary_text.append("-"*60)
    summary_text.append("STATISTICHE RIASSUNTIVE")
    summary_text.append("-"*60)
    summary_text.append(f"Gap medio: {avg_gap*100:.2f}%")
    summary_text.append(f"Accuratezza media: {avg_acc*100:.2f}%")
    ...
    print(f"\n✓ Report salvato in: {os.path.abspath(report_path)}")
    print(f"✓ Tutte le immagini salvate in: {abs_path}")

    # Lista i file salvati
    print(f"\nFile salvati:")
...
def interactive_test(predictor):
    ...
    while True:
        ...
        visualize_comparison(G, mst_pred, mst_true, filename)
        print(f"\nRisultato salvato come: {filename}")
...
def main():
    ...
    print(f"Controlla i file salvati nella directory: {os.getcwd()}")
    print("="*60)
